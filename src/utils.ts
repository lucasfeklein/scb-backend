...
export async function stripHtmlFromDocs(docs: any) {
...
// crawler
export async function crawlWebsite(homepageUrl: string) {
    try {
        const response = await axios.get(`${homepageUrl}/sitemap.xml`);
        const xmlData = response.data;
        const parser = new xml2js.Parser();
        const result = await parser.parseStringPromise(xmlData);
        const urls = result.urlset.url.map((item: any) => item.loc[0].replace(/\n/g, '').trim());
        console.log(urls);
        return urls;
    } catch (error) {
